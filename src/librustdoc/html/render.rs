...
pub struct Cache {
    ...
    /*/// Maps a type id to all known synthetic implementations for that type.
    /// Synthetic implementations are generated for auto traits (such as Send and Sync)
    /// when no user-provided impl exists. Any type without an explicit impl of an auto trait
    /// will always have a synthetic impl generated. The synthetic impl is positive if
    /// there is some set of type and lifetime parameters that makes the type implement
    /// the trait. Otherwise, it is negative
    ///
    /// It's important to distinguish between auto impls and synthetic impl. An auto
    /// impl is a legacy statement of the form 'impl Trait for ..', and physically exists
    /// in the crate. A synthetic impl is an impl generated by rustdoc, which doesn't
    /// correspond to any location in the crate. Its purpose is to document 
    /// the compiler-provided implementation of an auto trait, in the case where
    /// a user has not explicitly implemented the trait.
    pub synthetic_impls: FxHashMap<DefId, Vec<Impl>>*/
    ...
    /*/// Maps trait ids to known synthetic implementation of the trait.
    /// See 'synthetic_impls' for more details
    pub synthetic_implementors: FxHashMap<DefId, Vec<Implementor>>*/
...
fn render_impls(cx: &Context, w: &mut fmt::Formatter, traits: Vec<&&Impl>, containing_item: &clean::Item) -> Result<(), fmt::Error> {
    for i in &traits {
...
fn render_assoc_items(w: &mut fmt::Formatter,
    ...
    if !traits.is_empty() {
        ...
        let (synthetic, concrete) = traits.iter().partition::<Vec<_>, _>(|t| t.inner_impl().synthetic);
...
fn sidebar_assoc_items(it: &clean::Item) -> String {
    ...
    if let Some(v) = c.impls.get(&it.def_id) {
        ...
        if v.iter().any(|i| i.inner_impl().trait_.is_some()) {
            ...
            let (synthetic, concrete) = v.iter().partition::<Vec<_>, _>(|i| i.inner_impl().synthetic);
