...
impl<'a, 'tcx, 'rcx> AutoTraitFinder<'a, 'tcx, 'rcx> {
    ...
    fn find_auto_trait_generics(&self, did: DefId, trait_did: DefId, generics: &ty::Generics) -> AutoTraitResult {
        ...
        return tcx.infer_ctxt().enter(|mut infcx| {
            ...
            println!("Computed final full user env: {:?}", full_user_env);
                    ...
                    //if stop_at_param {
                        ...
                        if self.is_of_param(substs) && !only_projections {
                            if is_new_pred {
                                ...
                                println!("Skipping addition of non-fresh trait predicate {:?}", predicate);
                            }
                        } else {
                ...
                &ty::Predicate::Projection(p) => {
                    ...
                    if self.is_of_param(p.skip_binder().projection_ty.substs) /*|| !stop_at_param*/ {
                        ...
                        if is_new_pred {
                            ...
                            println!("Skipping addition of non-fresh projection predicate {:?}", predicate);
                        }
                        //} else {
    ...
    fn evaluate_predicates<'b, 'gcx, 'c>(&self, infcx: &mut InferCtxt<'b, 'tcx, 'c>, ty_did: DefId, trait_did: DefId, ty: ty::Ty<'c>, param_env: ty::ParamEnv<'c>, user_env: ty::ParamEnv<'c>,
        ...
        while let Some(pred) = predicates.pop_front() {
            ...
            //let fresh_preds = normalized_preds.map(|p| infcx.freshen(p));
            //println!("Lifting predicates: {:?} {:?}", ty, computed_preds);
            //println!("Normalized preds: {:?}", normalized_preds);

            new_env = ty::ParamEnv::new(tcx.mk_predicates(normalized_preds), param_env.reveal);
            println!("Env with fresh normalized predicates: {:?}", new_env);
        ...
        println!("User computed env: {:?}", final_user_env);
            ...
            match p {
                WherePredicate::BoundPredicate { ty, mut bounds } => {
                    ...
                    // this, as it would be a duplicate
                        ...
                        let mut replaced_trait_clean = replaced_trait.clean(self.cx);
