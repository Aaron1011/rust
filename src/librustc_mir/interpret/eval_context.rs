...
pub struct InterpretCx<'mir, 'tcx, M: Machine<'mir, 'tcx>> {
    ...
    /// Information about any in-progress panic unwinding
    pub unwinding: UnwindingState
}

/// Keeps track of relevant unwinding information,
/// across calls to pop_stack_frame
#[derive(Copy, Clone, Eq, PartialEq, Debug)]
pub enum UnwindingState {
    /// We are not unwinding. This is always the case in CTFE mode
    NotUnwinding,
    /// We are currently unwinding from a panic.
...
impl<'mir, 'tcx, M: Machine<'mir, 'tcx>> InterpretCx<'mir, 'tcx, M> {
    pub fn new(tcx: TyCtxtAt<'tcx>, param_env: ty::ParamEnv<'tcx>, machine: M) -> Self {
        InterpretCx {
            ...
            unwinding: UnwindingState::NotUnwinding
        }
    }
    ...
    pub(super) fn pop_stack_frame_internal(&mut self) -> InterpResult<'tcx, StackPopCleanup> {
        use UnwindingState::*;
        ...
        let prev_unwinding = self.unwinding;

        // Handle all possible state transitions
        self.unwinding = match (self.unwinding, stack_pop_info.unwinding) {
            // If we weren't previously unwinding, we start performing
            // unwinding (not in cleanup, since unwinding never starts at a
            // cleanup block)
            (NotUnwinding, true) => Unwinding { in_cleanup: false },
            // Nothing special here - we weren't unwinding before, and nothing changed
            (NotUnwinding, false) => NotUnwinding,
            // We were previously unwinding, and we want to keep unwinding
            (Unwinding { in_cleanup: true }, true) => Unwinding { in_cleanup: true },
            // We should never stop unwinding while in a cleanup block - cleanup
            // blocks are generated by the compiler, and should never 'catch'
            // an unwind
            (Unwinding { in_cleanup: true }, false) => {
                panic!("Attempted to stop unwinding during cleanup!")
            },
            // We want to keep unwinding - our cleanup state doesn't change,
            // since we haven't hit a terminator with a cleanup block
            (Unwinding { in_cleanup: false}, true) => Unwinding { in_cleanup: false },
            // The machine has indicated that unwinding is now complete
            (Unwinding { in_cleanup: false}, false) => NotUnwinding
        };
        info!("unwinding: state before {:?} from stack_pop {:?} after {:?}",
              prev_unwinding, stack_pop_info.unwinding, self.unwinding);
        ...
        match frame.return_to_block {
            ...
            StackPopCleanup::None { cleanup, .. } => {
                assert!(self.unwinding == NotUnwinding,
                        "Encountered StackPopCleanup::None while unwinding");

                if !cleanup {
                    assert!(self.stack.is_empty(), "only the topmost frame should ever be leaked");
                    // Leak the locals, skip validation.
                    return Ok(frame.return_to_block);
                }
            }
        }
        ...
        let do_normal_ret = self.unwinding != Unwinding { in_cleanup: false };
        trace!("unwind: performing normal return: {}", do_normal_ret);
        ...
        Ok(frame.return_to_block)
    }

    pub(super) fn pop_stack_frame(&mut self) -> InterpResult<'tcx> {
        use UnwindingState::*;
        let mut cleanup = self.pop_stack_frame_internal()?;
        ...
        if let Unwinding { in_cleanup: false } = self.unwinding {
            ...
            while !self.stack.is_empty() {
                if self.unwinding == NotUnwinding {
                ...
                match cleanup {
                    StackPopCleanup::Goto { unwind, .. } if unwind.is_some() => {
                        ...
                        self.unwinding = Unwinding { in_cleanup: true };
                ...
                cleanup = self.pop_stack_frame_internal()?;
            }
